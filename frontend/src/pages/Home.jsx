...
const Home = () => {
  ...
  return (
    <div className="home-container px-4 md:px-8 py-6">
      <h1 className="text-4xl font-semibold text-center text-indigo-700 mb-8">Welcome to the Counseling Platform</h1>
            ...
            featuredServices.map((service) => (
              <div key={service.id} className="service-card border p-6 bg-white rounded-lg shadow-lg transform transition duration-300 hover:scale-105 hover:shadow-2xl">
                ...
                <div className="mt-6 space-x-4">
                  <Link to={`/service/${service.id}`} className="btn btn-secondary bg-indigo-500 text-white px-6 py-2 rounded-md hover:bg-indigo-600">
                    Learn More
                  </Link>
      ...
      {/* Call to action for user profile and general appointment */}
      <div className="cta-buttons flex flex-col md:flex-row gap-4 mt-8">
        <Link to="/profile" className="btn btn-primary bg-blue-500 text-white px-6 py-2 rounded-md hover:bg-blue-600">
          View Profile
        </Link>
        <Link to="/appointment" className="btn btn-primary bg-green-500 text-white px-6 py-2 rounded-md hover:bg-green-600">
          Book an Appointment
